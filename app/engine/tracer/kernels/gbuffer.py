...
@ti.kernel
def gbuffer_kernel(albedo_f: ti.template(), normal_f: ti.template(), object_id_f: ti.template(), hit_mask_f: ti.template(), right: vec3, up: vec3, forward: vec3, position: vec3, fov: ti.f32, aspect_ratio: ti.f32, triangles: ti.template(), bvh_node_min: ti.template(), bvh_node_max: ti.template(), bvh_node_left: ti.template(), bvh_node_right: ti.template(), bvh_node_start: ti.template(), bvh_node_count: ti.template(), bvh_indices: ti.template()):  # type: ignore
  ...
  for px, py in albedo_f:
    ...
    hit = traverse_closest_hit(position, world_dir, bvh_node_min, bvh_node_max, bvh_node_left, bvh_node_right, bvh_node_start, bvh_node_count, bvh_indices, triangles)  # type: ignore

    if hit.hit == 0:
      # Background has no surface. Deliberately leave albedo and normal at
      # whatever clear_aovs() set -- writing plausible zeros invites
